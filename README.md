...
`dataSource`对象为集合视图配置cell和supplementary view时，使用`dequeueReusableCellWithReuseIdentifier:forIndexPath:`方法直接从重用队列中取cell，使用`dequeueReusableSupplementaryViewOfKind:withReuseIdentifier:forIndexPath:`方法直接从重用队列中取supplementary view。当重用队列中没有可复用的视图时，会自动帮我们新创建一个可用的视图。
...
集合视图插入，删除和移动cell之前，必须先对应更新数据源。如果数据源没有更新，程序运行就会崩溃。当插入，删除或者移动cell时，会自动添加动画效果来反映集合视图的更改。在执行动画时，如果还需要`同步`执行其他操作，可以使用`performBatchUpdates:completion:`方法，在`updates block`内执行所有插入，删除或移动调用，动画执行完毕后会调用`completion block`。
```
...
示例代码下载：https://github.com/zhangshijian/UICollectionViewDemo
